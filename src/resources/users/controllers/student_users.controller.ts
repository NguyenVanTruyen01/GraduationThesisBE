...
import { StudentUpdateUserDto } from '../dto/student_update-user.dto';
...
export class StudentUsersController {
  ...
  async updateProfileInformation(
    @Req() req: Request,
    @Body() updateUserDto: StudentUpdateUserDto
  ) {
    try {
  ...
  @Post('uploadFile')
  @UseInterceptors(FileInterceptor('user_file', { storage: storageConfig('user_file') }))
  async uploadFileForTopic(@Req() req: any,
    ...
    return await this.usersService.studentUploadFile(req, fileUrl);
  }
